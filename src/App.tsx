...
function BoardDetail({ boardId, profile }: { boardId: number; profile: Profile | null }) {
  ...
  const addMember = async (event: FormEvent) => {
    event.preventDefault()
    if (!memberEmail.trim()) { setError('Enter a valid email address.'); return }
    try { await api('/api/boardmember', { method: 'POST', body: JSON.stringify({ boardId, newMemberEmail: memberEmail.trim() }) }); setMemberEmail(''); await load() }
  ...
  const removeMember = async (member: Member) => { if (!window.confirm(`Remove ${member.userName} from this board?`)) return; try { await api(`/api/boardmember?boardId=${boardId}&memberId=${member.userId}`, { method: 'DELETE' }); await load() } catch (err) { setError(err instanceof Error ? err.message : 'Unable to remove member.') } }
...
function App() {
  ...
  const needsSignIn = !localStorage.getItem(tokenKey) && !publicPaths.includes(path)
  // Redirecting here (rather than during render) matters: navigate() dispatches a
  // popstate event, and the listener that turns that into a setPath() re-render is
  // only attached by the effect above once mounted. Calling navigate() synchronously
  // during the render body — as this used to — fires that event before the listener
  // exists, so the URL changes but the component is stuck rendering null forever.
  useEffect(() => { if (needsSignIn) navigate('/signin') }, [needsSignIn])
